...
int main(int argc, char *argv[])
{
   ...
   int recv_tim,tran_tim;
   int seqnum = 0;           //SEQ Number從0開始
   int buffersize = 32;      //預設Buffer Size為32 bytes
   struct hostent *hp;
   int rtt=0;
   char *hostip;
   int i, j;
   ...
   if(argc < 2) {            //輸入格式錯誤
      printf("Format: ./pingHW IP_Address\n");
      exit(0);
   }
   argc--;
   i = 1;
   ...
   for(i = 0; i < maxcount; i++) {
      ...
      s_time=(start_time.tv_sec%(24*60*60))*1000+start_time.tv_usec/1000;         //設定並且轉換時間格式
      ...
      recvfrom(sock, buffer,sizeof(buffer), 0, NULL, NULL);       //接收封包
